...
extractWords <- function( satznummer
	...
	file <- paste0("~/Desktop/output/", satznummer, ".csv")
...
extractFromAlignment <- function(satznummer, texts = alltexts, basefiles = allbasefiles) {
	...
	if (length(uniqueGroups > 0)) {		
		for (i in 1:length(uniqueGroups)) {
			...
			# export results
			name <- paste0("~/Desktop/QW/", satznummer, "_", n, "_", g, ".csv")
...
alltexts <- c("deu-x-bible-schlachter.txt")
...
sapply(allVerses[16:650], extractFromAlignment)
